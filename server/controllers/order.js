...
const createOrder = asyncHandler(async (req, res) => {
  ...
  for (const item of products) {
    ...
    if (!product) {
      return res.status(400).json({ success: false, mes: `Product not found: ${item.product}` })
    }
    if (product.quantity < item.quantity) {
      return res.status(400).json({ success: false, mes: `Sản phẩm '${product.title}' chỉ còn ${product.quantity} sản phẩm trong kho!` })
    }
  }
  // Nếu đủ tồn kho, cập nhật quantity và sold
  for (const item of products) {
